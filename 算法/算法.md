  ...
  - 0-1 Knapsack Algorithm
    - 负重一定，求最大价值 
      - 算法：
        ...
        $$  
      - 运行时间：设负重为W，O(nW) 
        - 问题大小为 O(log(W))，需要 log(W) 位来表示负重的值。但 O(nW)不是 log(W) 的多项式，是“伪多项式”。 
    - 所需价值一定，求最小负重 
      - 算法：
        $$
        OPT(i, v) =
        ...
        \end{cases}
        $$  
      - 运行时间：设物品价值最大值为v*，O(n^2v*)
        - 问题大小为 O(n log(v*))，需要 log(v*) 位来表示每个物品的值。但 O(n^2v*)不是 n log(v*) 的多项式，是“伪多项式”。
    - 两个问题是对偶问题，可以相互转化
      - 负重一定，求最大价值 => 所需价值一定，求最小负重
       \[
        W = \min_{w \le W} w \quad \text{s.t.} \quad \text{OPT}(n, w) \ge V
        \]  
      - 所需价值一定，求最小负重 => 负重一定，求最大价值
        \[
        V = \max_{v \le n v^*} v \quad \text{s.t.} \quad \text{OPT}(n, v) \le W
        \]
      - 对于背包问题，$$NP_{knappack} = Co-NP_{knappack}$$
      - 实际上，复杂度为\(O(n^2v^{*}+n^2w^{*}+nW+nV)\),但是\(n^2w^{*} \)一般不考虑（wi比较小），\(nV\)低于\(n^2v^{*}\)，则有\(O(n^2v^{*}+nW)\)。令价值为常值，则得到O(nW)；令重量为常值，则得到O(n^2v*)
    - 背包问题的决策版本属于NPC问题
    ...
    - 图像分割
      - 定义：
        ...
        - 前景/背景分割
          ...
          - ai >= 0 表示像素 i 属于前景的概率
          - bi >= 0 表示像素 i 属于背景的概率
          - pij >= 0 表示将 i 和 j 中的一个标记为前景、另一个标记为背景时的分离惩罚
        - 目标：
          - 准确性：若在孤立情况下 ai > bi，则优先将 i 标记为前景；
          - 平滑性：若 i 的许多邻域被标记为前景，则应倾向于将 i 标记为前景；
          - 寻找使以下条件最大化的划分 (A, B)：
            $$\sum_{i \in A} a_i + \sum_{j \in B} b_j - \sum_{\substack{(i,j) \in E \\ |A \cap \{i,j\}| = 1}} p_{ij}$$
      - 算法：
        - $$\sum_{i \in V} a_i + \sum_{j \in V} b_j$$为定值，则问题转化为求$$\sum_{i \in B} a_i + \sum_{j \in A} b_j + \sum_{\substack{(i,j) \in E \\ |A \cap \{i,j\}| = 1}} p_{ij}$$的最小值
        - 设(s,t),s到所有点的边值为ai，所有点到t的边值为bi，相邻点双向边的边值为pij，则该图的最小割即$$\sum_{i \in B} a_i + \sum_{j \in A} b_j + \sum_{\substack{(i,j) \in E \\ |A \cap \{i,j\}| = 1}} p_{ij}$$
    - 棒球淘汰赛
      - 定义：
        - 球队集合 S; 目标球队 z ∈ S; 球队 i 已获胜 wi 场; 球队 i 和 j 还将进行 gij 次交锋;剩余比赛是否存在某种结果，使得球队 z 最终获得最多（或并列最多）胜场?
        - wi + ri < wj 团队 i 被淘汰。充分条件，但非必要条件！
      - 算法：
        - 假设球队 z 的 gz 次交锋皆取胜，作如下图
         ![alt text](image-7.png)
        - 当最大流填满所有从源节点出发的边时，第z组不被淘汰    
        - 证明：由整体性定理，x 与 y 之间剩余的每场比赛都计入 x 队或 y 队的胜场数中；(x, t) 边上的容量确保没有一支队伍赢得过多比赛
        - [Hoffman-Rivlin 1967] 组 z 被淘汰当且仅当存在子集 T* 使得$$\overbrace{\frac{w(T^{*}) + g(T^{*})}{|T^{*}|}}^{\text{Load Balance on avg\# games won}} > w_z + g_z$$
        - 证明:
          - <= T* 中各队的平均胜场数大于 z 的最大胜场数
          - => 
            - 最大流建模，并考虑最小割 (A, B)
            - 定义 T* = 最小割源侧的球队节点
            -  x-y ∊ A 当且仅当 x ∊ T* 且 y ∊ T*
               - 无限容量的边保证：若 x-y ∊ A，则 x ∊ A 且 y ∊ A
               - 若 x ∊ A 且 y ∊ A，但 x-y ∊ B，则将 x-y 加入 A 会降低割的容量，矛盾
            - 由于 z 被淘汰，根据最大流-最小割定理：
             $$
              g(S - \{z\}) > cap(A,B) 
              ...
              = g(S - \{z\}) - g(T^{*}) - w(T^{*}) + |T^{*}|(w_z + g_z) \\
               \quad w_z + g_z < \frac{w(T^{*}) + g(T^{*})}{|T^{*}|}.
             $$
    - 项目选取
      - 定义：
        - 带有先决条件的项目集合 P。项目 v 对应的收入为 pv
        - 部分项目能产生收益：创建电子商务界面、设计网页
        - 部分项目需要支出：升级计算机、购买网站许可证
        - 先决条件集 E。若 (v, w) ∊ E，则除非执行项目 w，否则无法执行项目 v（w 是 v 的必要条件）
        - 若项目子集 A ⊆ P 中每个项目的先决条件也属于 A，则该子集为可行集
        - 选择一个可行的项目子集以最大化收益
      - 算法：
        - 最小割模拟：如果(v, w) ∊ E，则应包含从 v 到 w 的边，分配容量 ∞
        - 若 pv > 0，则添加容量为 pv 的边 (s, v)
        - 若 pv < 0，则添加容量为 –pv 的边 (v, t)
        - 为方便记号，定义 ps = pt = 0
        - (A, B) 是最小割当且仅当 A – { s } 是项目子集的最优解。
          - 无限容量的边确保 A – { s } 满足E，因为最小割要求A包含v时也包含w
          - 命题成立，因为：
            $$
              \mathrm{cap}(A,B)
              = \sum_{v\in B: p_v>0} p_v + \sum_{v\in A: p_v<0} (-p_v) \\
              = \sum_{v\in B: p_v>0} p_v + \sum_{v\in A: p_v>0} p_v- \sum_{v\in A: p_v>0} p_v + \sum_{v\in A: p_v<0} (-p_v) \\
              = \underbrace{\sum_{v: p_v>0} p_v}_{\text{constant}} - \sum_{v\in A} p_v
            $$
      - 应用：
        - 露天开采（自20世纪60年代初开始研究）
          - 从地表开采土块以获取矿石
          - 每个土块v的净值pv = 矿石价值 – 加工成本
          - 不能在移除土块w或x之前移除土块v                          