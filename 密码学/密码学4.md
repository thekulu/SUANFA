...
- 公钥加密
  - Gen, Enc, Dec：密钥生成、加密、解密
  - m, c, pk, sk：明文(消息)、密文、公钥、私钥
  - M, C, K：明文空间、密文空间、密钥空间
  - \(\Pi=(Gen, Enc, Dec)+\mathcal{M}, |\mathcal{M}|>1\)
  - 正确性：对任意pk, sk, m，有\(Dec(sk, Enc(pk, m))=m\)
- 私钥与公钥加密对比
  - 密钥数量
    - 私钥加密：加密和解密使用相同密钥
    - 公钥加密：加密和解密使用不同密钥
  ...
  - 密钥分发
    ...
    - 公钥加密：公开pk即可
  - 密钥存储
    - 私钥加密：N个用户需要N(N-1)/2个密钥
    - 公钥加密：N个用户需要N个公钥和N个私钥
  - 发送者/接收者数量
    - 私钥加密：1个发送者，1个接收者
    - 公钥加密：多个发送者，1个接收者
  - 谁加密/解密
...
- 朴素RSA签名
  ...
  - 安全性：朴素RSA签名不是CMA安全的
    - 无查询攻击：
      ...
      - 输出：$(m, \sigma)$
    - 单次查询攻击：
      - 输入：公钥 $pk=(N,e)$，消息 $m_1 \in \mathbb{Z}_N^*$
        1. 随机选取 $\sigma_0$，计算 $m_0 = \sigma_0^e \pmod{N}$
        2. 使用 $m_1$ 调用签名预言机 $\text{Sign}(sk,\cdot)$，得到签名 $\sigma_1 = m_1^d \pmod{N}$
        3. 令 $\sigma = (\sigma_0 \sigma_1) \pmod{N}$
      - 输出：$(m_0 m_1,\ \sigma)$
      - 验证
        $$
        \begin{aligned}
        \sigma^e &\equiv (\sigma_0 \sigma_1)^e \pmod{N} \\
        &\equiv \sigma_0^e \cdot (\sigma_1)^e \pmod{N} \\
        &\equiv m_0 \cdot \left(m_1^d\right)^e \pmod{N} \\
        &\equiv m_0 m_1^{de} \pmod{N} \\
        &\equiv m_0 m_1 \pmod{N}
        \end{aligned}
        $$
...
- 私有信息检索(Private Information Retrieval, PIR)
  - 问题：Bob存数据库$x_1\dots x_n$，Alice要获取$x_i$且隐藏下标$i$
    - 朴素构建：令$n=\ell^2$，数据转为$\ell\times\ell$矩阵$X$，$x_i=X_{u,v}$
      - Alice操作：
    ...
    - 三维低通信高数据库隐私构建：令$n=\ell^3$，数据转为$\ell\times\ell\times\ell$三维立方体$X$，$x_i=X_{u,v,w}$
      - Alice操作：
        - 生成Paillier公私钥$(pk=N,sk=(p,q))$；
        - 构造三组共$3\ell$个密文选择向量：
          - u维向量$\{c_u^{(k)}\}_{k=1}^\ell$：$k\neq u$时$c_u^{(k)}=r_k^N\pmod{N^2}$（明文0）；$k=u$时$c_u^{(k)}=(1+N)r_k^N\pmod{N^2}$（明文1）
          - v维向量$\{c_v^{(k)}\}_{k=1}^\ell$：$k\neq v$时$c_v^{(k)}=s_k^N\pmod{N^2}$（明文0）；$k=v$时$c_v^{(k)}=(1+N)s_k^N\pmod{N^2}$（明文1）
          - w维向量$\{c_w^{(k)}\}_{k=1}^\ell$：$k\neq w$时$c_w^{(k)}=t_k^N\pmod{N^2}$（明文0）；$k=w$时$c_w^{(k)}=(1+N)t_k^N\pmod{N^2}$（明文1）
        - 发送三组密文$\{c_u^{(k)}\},\{c_v^{(k)}\},\{c_w^{(k)}\}$
      - Bob操作
        - 沿w维聚合：对每个固定$(a,b) \in [\ell]\times[\ell]$，计算中间密文$d_{a,b} = \prod_{k=1}^\ell \left(c_w^{(k)}\right)^{X_{a,b,k}} \pmod{N^2}$，得到$\ell\times\ell$个中间结果
        - 沿v维聚合：对每个固定$a \in [\ell]$，计算中间密文$e_a = \prod_{k=1}^\ell \left(c_v^{(k)}\right)^{d_{a,k}} \pmod{N^2}$，得到$\ell$个中间结果
        - 沿u维聚合：计算最终结果密文$f = \prod_{k=1}^\ell \left(c_u^{(k)}\right)^{e_k} \pmod{N^2}$
        - 发回Alice 1个最终密文$f$
      - Alice解密
        - $f=(1+N)^{X_{u,v,w}} \cdot \left(\prod \text{随机因子项}\right)^N \pmod{N^2}$，$\text{Dec}(sk,f)=X_{u,v,w}=x_i$
      - Alice仅获取目标数据$x_i$，无法得知其余任何数据库条目
      - 通信复杂度：$3\ell + 1 = O(n^{1/3})$个$\mathbb{Z}_{N^2}^*$元素
...
- 交易类型1：支付到公钥哈希（Pay to public key hash，P2PKH）
  ...
  - Bob发布交易Tx2（spending Tx）
    - in（5 BTC）是$UTXO_B$即(Tx1,0)的指针，附加$ScriptSig_B$，授权花费Bob的UTXO
    - $ScriptSig_B$ = $<sig><pk_B>$
      $$<sig> = \text{Sign}(sk_B, \text{Tx})$$
      - Tx代表删去所有ScriptSig的Tx2（SIGHASH_ALL），包含(Tx1,0)与Tx2的out
  - 矿工验证$ScriptSig_B | ScriptPK_B$返回真
    - 比特币脚本语言：基于栈，指令线性执行
      1. 压入签名`<sig>`、公钥`<pk_B>`
      2. `OP_DUP`：复制公钥，栈新增一份`<pk_B>`
      3. `OP_HASH160`：对公钥哈希，替换栈顶为公钥哈希`<addr_B>`
      4. 压入锁定脚本内置地址`<addr_B>`，栈顶出现两组地址哈希
      5. `OP_EQUALVERIFY`：比对两组哈希，匹配则删除两个哈希，栈只剩`<sig> <pk_B>`
      6. `OP_CHECKSIG`：校验签名，合法则栈顶输出`true`
  - Alice在$UTXO_B$中指定接收者的公钥哈希；接收者公钥直到UTXO被接收者花费才暴露
  - 恶意矿工无法伪装Bob，修改$ScriptPK_B$中的$<addr_B>$盗取币
    - 诚实矿工验证时，修改后的地址$<addr_B'>$无法通过`OP_EQUALVERIFY`，会使$<sig>$失效
- 交易类型2：支付到脚本哈希（Pay to script hash, P2SH）
  - Alice将BTC发送到一个赎回脚本的哈希
    ...
    - UTXO(Tx1)中的ScriptPK
      ...
      - $<addr>$
      - OP_EQUAL
    - Tx2的ScriptSig：$$<sig_1><sig_2>\cdots<sig_n><\text{redeem script}>$$
  - 矿工验证两件事
    - ScriptSig | ScriptPK = true：花费交易Tx2给出了正确脚本
    - ScriptSig = true：赎回脚本被满足
  - Ex：多重签名，花费UTXO需要2-out-of-3签名
    - 赎回脚本：
      - OP_2 $<pk_1><pk_2><pk_3>$ OP_3 OP_CHECKMULTISIG
    - UTXO(Tx1)中的ScriptPK
      - OP_HASH160
      - $<addr>$
      - OP_EQUAL
    - Tx2的ScriptSig：OP_0 $<sig_1><sig_3><\text{redeem script}>$
    - 矿工验证 $ScriptSig | ScriptPK$ 返回真
      1. 依次压入：$OP_0$、$<sig_1>$、$<sig_3>$、完整赎回脚本
      2. `OP_HASH160`：弹出栈顶赎回脚本，计算哈希后压回栈
      3. 压入锁定脚本内置哈希地址 $<addr>$
      4. `OP_EQUAL`：对比栈顶两组哈希，匹配则栈输出 `true`
    - 矿工执行ScriptSig，返回真
      1. 压入占位符 $OP_0$（弥补CHECKMULTISIG栈偏移漏洞）
      2. 依次压入有效签名 $<sig_1>$、$<sig_3>$
      3. 依次压入赎回脚本参数：`OP_2`、$<pk_1>$、$<pk_2>$、$<pk_3>$、`OP_3`
      4. `OP_CHECKMULTISIG`：校验3个公钥中存在2个有效签名，通过输出 `true`
- P2SH 对比 P2PKH 的优势
  - 支持复杂逻辑脚本
    - P2PKH：锁定脚本固定只能单签验证，仅支持「一个私钥签名解锁
    - P2SH：底层可承载任意赎回脚本，原生支持多重签名、限时合约、条件转账等复杂逻辑，典型场景2-out-of-3签名
  - 对外地址统一简洁，兼容所有钱包
    - 若直接把多签赎回脚本写进ScriptPubKey（不使用P2SH），转账地址会是超长字符串，旧钱包无法识别、无法转账。
    - P2SH仅对外暴露一段20字节哈希地址，格式和普通P2PKH地址一致，任意比特币钱包均可正常转账，无需特殊适配
  - 锁定阶段隐藏复杂逻辑，保护隐私
    - P2PKH的锁定逻辑固定公开，一眼能看出是单签地址
    - P2SH转账时仅存赎回脚本哈希，链上看不到完整赎回脚本；第三方无法通过地址判断是多签还是普通合约，隐藏资金解锁规则，提升隐私性
  - 降低链上存储开销，节约手续费
    - 锁定UTXO时仅存储20字节哈希，而非完整冗长的赎回脚本，每笔注资交易体积更小，手续费更低。
    - 完整赎回脚本仅在花费UTXO时才上链，多笔资金存入同一脚本哈希时，仅一次存储复杂脚本。
  - 拓展性强，实现各类链上合约
    - 基于P2SH可构建托管、多签金库、原子交换、到期解锁等智能合约逻辑；P2PKH无拓展能力，仅能完成基础一对一转账
