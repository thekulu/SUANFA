...
- 比特币交易（非coinbase 交易）
  ...
  - in
    - prev_out：引用前一笔交易的输出，定位历史 UTXO
      ...
      - n：该输出在交易中的索引，4字节
      - scriptSig
        - 解锁脚本（输入脚本），该prev_out所有者的签名，证明接收者拥有这笔 UTXO 的所有权
...
- 交易验证
  - 输入（Tx1, 1）在当前UTXO集合中
  - 输入价值总和 ≥ 输出价值总和（差额为交易手续费）
  - 脚本ScriptSig | ScriptPK执行结果为真
- 交易类型1：支付到公钥哈希（P2PKH）
  - 示例：Alice用一个7 BTC的UTXO向Bob支付5 BTC
  - Bob生成签名密钥对$$(pk_B, sk_B)$$
  - Bob计算比特币地址$$addr_B = H(pk_B)$$
  - Bob将$$addr_B$$发送给Alice
  - Alice发布交易Tx1
    - 输入（7 BTC）包含ScriptSig，授权花费Alice的UTXO
    - ScriptSig包含Alice对交易的签名
    - 矿工无法修改ScriptPK_B（会使Alice签名失效）
  - 输出：5 BTC给Bob（ScriptPK_B），2 BTC找零给Alice（ScriptPK_A）
  - ScriptPK脚本：OP_DUP OP_HASH160 $$<addr_B>$$ OP_EQUALVERIFY OP_CHECKSIG
  - Bob花费UTXO_B：发布Tx2，ScriptSig包含$$<sig><pk_B>$$
  - $$<sig> = \text{Sign}(sk_B, \text{Tx})$$，Tx为排除所有ScriptSig的交易（SIGHASH_ALL）
  - 矿工验证ScriptSig_B | ScriptPK_B返回真
  - 比特币脚本语言：基于栈，指令线性执行
    - 数据指令：将数据压入栈顶
    - 操作码：对栈顶输入数据执行函数
    - OP_DUP：将公钥副本压入栈顶
